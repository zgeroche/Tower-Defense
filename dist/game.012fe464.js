...
function buildMap(scene) {
  ...
  scene.add.image(320, 256, 'map'); //add background music
...
function (_Phaser$GameObjects$S) {
  ...
  function Enemy(scene) {
    ...
    _this.hp = 0;
    _this.turned = 0; //this.text = scene.add.text(0, 0, "HP: "+ ENEMY_HP, {font: "16px Arial", fill: "#ffffff"});
...
function (_Enemy) {
  ...
  function Deathknight(scene) {
    ...
    Phaser.GameObjects.Sprite.call(_assertThisInitialized(_assertThisInitialized(_this2)), scene, 0, 0, 'deathknight');

    _this2.anims.play('dkdown'); //create sounds


    _this2.death = scene.sound.add('dkDeath');
    _this2.death.volume = 0.05;
    _this2.death.loop = false;
    _this2.damage = scene.sound.add('hit');
...
function (_Phaser$GameObjects$S2) {
  ...
  function Tower(scene, stats) {
    ...
    _this3.text = scene.add.text(0, 0, _this3.towerName, {
      font: "16px Arial",
      fill: "#ffffff"
    });
    _this3.upgradeSound = scene.sound.add('upgradeSound');
  ...
  _createClass(Tower, [{
    key: "placeTower",
    value: function placeTower(pointer, scene) {
      var i = Math.floor(pointer.y / 64);
      var j = Math.floor(pointer.x / 64);
      ...
      if (map[i][j] === 0) {
        ...
        if (this) {
          ...
          this.text.setOrigin(0.5); //scene.add.container([this.text, this]);
    ...
    key: "upgradeTower",
    value: function upgradeTower(pointer, newTower, scene) {
      //check if tower is already upgraded, if not the upgrade
      if (this.towerName !== newTower.towerName) {
        var i = Math.floor(pointer.y / 64);
        var j = Math.floor(pointer.x / 64);
        this.removeTower(pointer);
        newTower.placeTower(pointer, scene); //console.log(TOWER_GROUP.getTotalUsed());
        //console.log(TOWER_GROUP.getLength());
...
function (_Phaser$GameObjects$I) {
  ...
  function Attack(scene) {
    ...
    _this6.particles = scene.add.particles('attack');
...
function (_Phaser$Scene) {
  ...
  _createClass(GameScene, [{
    ...
    value: function create() {
      ...
      buildMap(this); //input related actions in userAction function

      this.input.on('pointerdown', function (pointer) {
        userAction(pointer, this);
      }); //create animations

      this.anims.create({
        key: 'dkdown',
        frames: this.anims.generateFrameNames('deathknight', {
          prefix: 'walk_down_',
          start: 1,
          end: 4
        }),
...
function (_Phaser$Scene) {
  ...
  _createClass(LoadScene, [{
    ...
    value: function preload() {
      ...
      this.load.atlas('peasant', './assets/peasant.png', 'assets/peasant.json'); //load enemy sprites, will be a loop like load tower sprites

      this.load.atlas('deathknight', 'assets/deathknight.png', 'assets/deathknight.json'); //load map
      ...
      this.load.audio('upgradeSound', 'assets/Sounds/General Sounds/Positive Sounds/sfx_sounds_powerup6.wav'); //load misc

      this.load.image('attack', 'assets/coin.png');
      this.load.image('menuscreen', './assets/bg.png', {
        frameWidth: 640,
        frameHeight: 512
      });
      this.load.image('newgame', './assets/new.png');
